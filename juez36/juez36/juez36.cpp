...
private:
    ...
    static int count_interval(Node* node, T const& lower, T const& upper) {
        if (node == nullptr) {
            return 0;
        }
        else if (node->elem < lower) {
            return count_interval(node->right, lower, upper);
        }
        else if (node->elem > upper) {
            return count_interval(node->left, lower, upper);
        }
        else {
            int contador = 1;
            if (node->elem < upper) {
                contador += count_interval(node->right, lower, upper);
            }
            if (node->elem > lower) {
                contador += count_interval(node->left, lower, upper);
            }
            return contador;
        }
...
int SetTree<T>::count_interval(T const& lower, T const& upper) const {
    int s2 = count_interval(root_node, lower, upper);
    return s2;
}
