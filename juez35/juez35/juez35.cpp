...
/*@ <answer>
 ...
 //Devuelve si el arbol es de busqueda, el minimo y el maximo del arbol
template <typename T>
pair<bool, pair<T, T>> busqueda(BinTree<T> const& t) {
    if (t.left().empty() && t.right().empty()) return { true, {t.root(), t.root()} };
    else if (t.left().empty()) {
        auto [ok, par] = busqueda(t.right());
        return { ok && (t.root() < par.first), {t.root(), par.second} };
    }
    else if (t.right().empty()) {
        auto [ok, par] = busqueda(t.left());
        return { ok && (par.second < t.root()), {par.first, t.root()} };
    }
    else {
        auto [okI, parI] = busqueda(t.left());
        auto [okD, parD] = busqueda(t.right());
        bool ok = okI && okD && (parI.second < t.root() && t.root() < parD.first);
        return { ok, {parI.first, parD.second} };
    }
}

template <typename T>
bool resolver(BinTree<T> const& t) {
    if (t.empty()) return true;
    else return busqueda(t).first;
}
...
bool resuelveCaso() {
    ...
    if (c == 'N') {
        if (resolver(read_tree<int>(std::cin))) cout << "SI\n";
        else cout << "NO\n";
    }
    else {
        if (resolver(read_tree<string>(std::cin))) cout << "SI\n";
        else cout << "NO\n";
    }
    return true;
}
