...
template <typename T> class SetTree {
    ...
    private:
        ...
        std::pair<bool, T> sol(Node* root, T const& elem) const;
};
...
template <typename T>
std::pair<bool, T> SetTree<T>::sol(Node* root, T const& elem) const {
    if (root == nullptr) return { false, T() };
    if (root->elem < elem) return sol(root->right, elem);
    else {
        auto [ok, candidato] = sol(root->left, elem);
        if (ok) return { ok, candidato };
